...
contains
    ...
    module procedure datenum0
        ...
        if ((month .lt. 1) .and. (month .gt. 12)) then
            call disp("Error: month should be between 1 and 12 ("//format_string(month)//").")
        end if
        if ((day .lt. 1) .and. (day .gt. 31)) then
            call disp("Error: day should be between 1 and 31 ("//format_string(day)//").")
        end if
        if ((present(hour)) .and. (hour .lt. 0) .and. (hour .gt. 23)) then
            call disp("Error: hour should be between 0 and 23 ("//format_string(hour)//").")
        end if
        if ((present(minute)) .and. (minute .lt. 0) .and. (minute .gt. 59)) then
            call disp("Error: minute should be between 0 and 59 ("//format_string(minute)//").")
        end if
        if ((present(second)) .and. (second .lt. 0) .and. (second .gt. 59)) then
            call disp("Error: second should be between 0 and 59 ("//format_string(second)//").")
        end if
        if ((present(microsecond)) .and. (microsecond .lt. 0) .and. (microsecond .ge. 1.0d+6)) then
            call disp("Error: microsecond should be between 0 and 999,999 ("//format_string(microsecond)//").")
        end if
